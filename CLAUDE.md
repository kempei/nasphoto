...
以下の順序を**厳守**する。前のステップが完了していない状態で次のステップの実装を開始してはならない。

```
Step 1: db      ← すべての基盤。スキーマが固まるまで他を始めない
Step 2: omvapi  ← db と並行可。iOS/macOS の前提となるシークレット管理・APIを先に固める
Step 3: macos   ← db + omvapi が動作する状態になってから着手
Step 4: ios     ← omvapi 経由の全機能が揃ってから着手
```
...
| ステップ | 完了条件 |
|---|---|
| db | Supabase にスキーマ適用済み・RLS 設定済み・テストデータで検索クエリが動作する |
| omvapi | `/health` エンドポイント稼働・JWT 認証通過・Parameter Store からシークレット取得できる |
| macos | バックアップ → Supabase メタデータ保存のフローが E2E で動作する |
| ios | ギャラリー表示・検索が omvapi 経由で動作する |

### 理由

- **db が最優先**: スキーマ変更はすべてのコンポーネントに波及するため、最初に固める
- **omvapi が第2**: Go のため Xcode 不要で即テスト可能。シークレット管理は macOS/iOS の前提
- **macOS が第3**: omvapi と db の両方が動いていると本物のフローで検証できる
- **iOS が最後**: omvapi 経由でほぼすべてが揃った状態でないと統合テストが難しい
